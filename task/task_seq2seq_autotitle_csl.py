...
# 加载数据集
train_data = load_data('dataset/csl_summary/train.tsv')
valid_data = load_data('dataset/csl_summary/val.tsv')
...
def show():
    samples = sample(valid_data, 10)

    for each in samples:
        title = each[0]
        passage = each[1]
        g_title = autotitle.generate(text=passage)
        print('passage:')
        print(passage)
        print('origin title:')
        print(title)
        print('generative title:')
        print(g_title)
...
class Evaluator(keras.callbacks.Callback):
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        if metrics['bleu'] > self.best_bleu:
            ...
            model.save_weights('/home/jiangweiwei/pretrained-unilm-Chinese/output/autotitle_csl/best_model.weights')  # 保存模型
